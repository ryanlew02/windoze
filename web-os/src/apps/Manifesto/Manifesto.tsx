...
const MANIFESTOS: Record<FactionId, string> = {
  dauntless: `We believe that cowardice is to blame for the world's failure.
...
Cowardice is a choice. So is bravery. We have made ours.`,

  erudite: `The pursuit of knowledge is the highest calling of the human mind.
...
Knowledge is not power. Knowledge is the prerequisite for every power worth having. The one who understands the world will always outlast the one who only knows how to fight it.`,

  amity: `We believe that conflict — all conflict — is a choice.
...
The fruit of peace is a longer harvest than the fruit of war.`,

  abnegation: `The self is the enemy.
...
We ask for nothing. We expect nothing. What we receive is beside the point.`,

  candor: `Lies are the foundation of every atrocity.
...
The world does not need more diplomacy. It needs more people willing to say the thing that everyone already knows but no one will admit. We are those people. We will not pretend otherwise.`,

  divergent: `There are those who do not fit.
...
They call us dangerous. They are correct.`,
};

export function ManifestoApp() {
  const currentFactionId = useThemeStore((s) => s.factionId);
  const [selected, setSelected] = useState<FactionId>(
    currentFactionId === 'divergent' ? 'divergent' : currentFactionId,
  );
    ...
    <div className={styles.container}>
      <div className={styles.sidebar}>
        {factions.map((f) => (
          <button
            key={f.id}
            className={`${styles.sideItem} ${f.id === selected ? styles.sideItemActive : ''}`}
            style={f.id === selected ? { borderLeftColor: f.color, color: f.color } : undefined}
            onClick={() => setSelected(f.id)}
          >
            <span className={styles.sideSymbol}>{f.symbol}</span>
      ...
      <div className={styles.content}>
        <div className={styles.header}>
          <span className={styles.headerSymbol}>{selectedFaction.symbol}</span>
          <h2 className={styles.headerName} style={{ color: selectedFaction.color }}>
            {selectedFaction.name.toUpperCase()}
          </h2>
        </div>
        <div className={styles.divider} style={{ background: selectedFaction.color }} />
        <p className={styles.body}>{MANIFESTOS[selected]}</p>
      </div>
    </div>
  );
}
